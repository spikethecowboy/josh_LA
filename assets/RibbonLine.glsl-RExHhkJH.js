...
return animatedColor;
        ...
        return (exp(-decayRate * x) - cutOff) / (1.0 - cutOff);`;default:return"return 1.0;"}}const Pa=Nt(),Ye=1;function $t(t){const e=new Hi,{attributes:a,varyings:i,vertex:n,fragment:r}=e,{applyMarkerOffset:l,draped:d,output:o,capType:c,stippleEnabled:p,falloffEnabled:m,roundJoins:v,wireframe:f,innerColorEnabled:h,hasAnimation:N,hasScreenSizePerspective:g,worldSizedImagePattern:C}=t;n.inputs.add("position",()=>"position"),r.include(Ui),e.include(la,t),e.include(ua,t),e.include(oi,t),e.include(za,t);const B=l&&!d;B&&(n.uniforms.add(new X("markerScale",u=>u.markerScale)),e.include(xa,{space:2,hasScreenSizePerspective:g})),li(n,t),n.uniforms.add(new ci("inverseProjectionMatrix",u=>u.camera.inverseProjectionMatrix),new di("nearFar",u=>u.camera.nearFar),new X("miterLimit",u=>u.join!=="miter"?0:u.miterLimit),new Bi("viewport",u=>u.camera.fullViewport)),n.constants.add("LARGE_HALF_FLOAT","float",65500),n.constants.add("EPS","float",.001),n.constants.add("NUM_JOIN_SUBDIVISIONS","float",t.numJoinSubdivisions),a.add("position","vec3"),a.add("previousDelta","vec4"),a.add("nextDelta","vec4"),a.add("lineParameters","vec2"),a.add("u0","float"),i.add("vColor","vec4"),i.add("vpos","vec3",{invariant:!0}),i.add("vLineDistance","float"),i.add("vLineWidth","float"),p||(i.add("vIsInsideJoin","int"),i.add("vStretchFactor","float"),i.add("vJoinCenterLineSDFs","vec2"),i.add("vSubdivisionFactor","float"));const q=p;q&&i.add("vLineSizeInv","float");const b=c===2,x=p&&b,R=m||x;R&&i.add("vLineDistanceNorm","float"),b&&(i.add("vSegmentSDF","float"),i.add("vReverseSegmentSDF","float")),n.code.add(s`vec3 perpendicular(vec3 v) {
...
color.a = noPerspectiveRead(color.a);`),h&&r.uniforms.add(new ke("innerColor",u=>u.innerColor??u.color),new X("innerWidth",(u,$)=>u.innerWidth*$.camera.pixelRatio)).main.add(s`float distToInner = abs(lineDistance) - innerWidth;
